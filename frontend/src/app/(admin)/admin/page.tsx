...
export default function AdminDashboard() {
  ...
  const pieData = Object.entries(stats?.gender_distribution || {}).map(
    ([name, value]) => ({
      name,
      value,
    }),
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground font-medium">
            Berikut adalah rangkuman data akademik hari ini.
          </p>
        </div>
        ...
        <motion.div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {statCards.map((stat) => (
            <motion.div key={stat.title} variants={item}>
              <Card
                className={cn(
                  "overflow-hidden border-none shadow-none glass-card group cursor-pointer h-full",
                )}
              >
                <div
                  className={cn(
                    "absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 rounded-full blur-3xl opacity-20 transition-all duration-500 group-hover:scale-150",
                    stat.bg,
                  )}
                />
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                  ...
                  <div
                    className={cn(
                      "p-2.5 rounded-xl border-2 shadow-sm transition-all duration-300 group-hover:scale-110",
                      stat.bg,
                      stat.border,
                    )}
                  >
            ...
            <Card className="border-none shadow-none glass-card overflow-hidden">
              <CardHeader className="pb-0 pt-6 px-6">
                ...
                <p className="text-sm text-muted-foreground font-medium">
                  Perbandingan data master sistem
                </p>
              </CardHeader>
          ...
          <motion.div className="lg:col-span-3" variants={item}>
            <Card className="border-none shadow-none glass-card h-full overflow-hidden">
              <CardHeader className="pb-4 pt-6 px-6">
                ...
                <p className="text-sm text-muted-foreground font-medium">
                  Berdasarkan data siswa terdaftar
                </p>
              </CardHeader>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground font-medium ml-1">
                Pantau informasi yang baru saja dipublikasikan.
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            {beritaLoading ? (
              Array(3)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="h-48 rounded-[2rem] bg-muted animate-pulse border border-border/50"
                  />
                ))
            ) : berita?.length > 0 ? (
                    ...
                    <Card className="flex flex-col h-full group hover:border-primary/50 transition-all duration-300 bg-card/40 backdrop-blur-sm rounded-3xl overflow-hidden border-border/50 shadow-sm hover:shadow-xl">
                      ...
                      <CardContent className="p-6 flex flex-col justify-between flex-1">
                        ...
                        <div className="pt-4 mt-4 border-t border-border/50 flex items-center justify-between">
                          ...
                          <span className="text-[10px] font-black p-1 px-2 rounded-lg bg-primary/10 text-primary uppercase">
                            UID: {news.id_user}
                          </span>
                        </div>
                      </CardContent>
              ...
              <div className="col-span-full py-12 flex flex-col items-center justify-center gap-4 bg-muted/20 rounded-[2rem] border border-dashed border-border/50">
                ...
                <p className="text-muted-foreground font-bold italic">
                  Belum ada berita terbaru hari ini.
                </p>
              </div>
            )}
          </div>
